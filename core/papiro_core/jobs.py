...
def job_atualizar(job_id: str, status: str, passo: int | None = None, erro: str | None = None,
                  motor: str | None = None, segundos: float | None = None):
    with _con() as c:
        c.execute("""UPDATE jobs SET status=?, passo=COALESCE(?, passo), erro=?, motor=COALESCE(?, motor),
                     segundos=COALESCE(?, segundos), atualizado=? WHERE id=?""",
                  (status, passo, erro, motor, segundos, utcnow_iso(), job_id))


def job_status(job_id: str) -> dict:
    with _con() as c:
        r = c.execute("SELECT ferramenta, op, status, passo, plano, criado, atualizado, erro, motor, segundos "
